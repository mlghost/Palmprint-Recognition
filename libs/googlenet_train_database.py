...
last_conv = net.layers['inception_5b_output']
last_conv = tf.reshape(last_conv, (-1, 7 * 7 * 1024))


w = tf.Variable(tf.random_normal(shape=[7 * 7 * 1024, 54], mean=0, stddev=2 / np.sqrt(7 * 7 * 1024)), name='last_W')
b = tf.Variable(tf.zeros([54]), name='last_b')
output = tf.matmul(last_conv, w) + b
...
epoch = 40
save_path = './log/weights/googlenet_database_fine_tune.ckpt'
log_path = './log/summaries/googlenet_database/'
extract = False
step = 0
loadw = False
train = True
    ...
    if train:
        ...
        for i in range(epoch):
            ...
            for j in range(len(train) / batch_size):
                ...
                if step % 5 == 0:
                    random.shuffle(test)
                    xbt, ybt = [test[p][0] for p in range(12)], \
                               [test[p][1] for p in range(12)]
