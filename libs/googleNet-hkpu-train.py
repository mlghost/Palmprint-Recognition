...
w = tf.Variable(tf.random_normal(shape=[100, 386], mean=0, stddev=2 / np.sqrt(100)), name='last_W')
...
epoch = 40
save_path = './log/weights/googlenet-HKPU-fine_tune.ckpt'
log_path = './log/summaries/hkpu/'
extract = True
step = 0
loadw = True
train = False
...
with tf.Session() as sess:
    ...
    if loadw:
        ...
        im_path = './data/test/hkpu/'
        if extract:
            images = [
                [mp.imread('./data/test/hkpu/' + name), get_class_number(name), name]
                for name in os.listdir('./data/test/hkpu/')
            ]
            features = []
            ...
            features.to_csv('GoogleNet_features_HKPU_test.csv', index=False)
    ...
    if train:
        images = [
            [mp.imread('./data/hkpu/' + name),get_class(get_class_number(name))]
            for name in os.listdir('./data/hkpu')
        ]
