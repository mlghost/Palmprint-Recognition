...
net = VGG_ILSVRC_19_layer({'data': image})

last_conv = net.layers['pool5']
last_conv = tf.reshape(last_conv, (-1, 7 * 7 * 512))

w = tf.Variable(tf.random_normal(shape=[7 * 7 * 512, 54], mean=0, stddev=2 / np.sqrt(7 * 7 * 512)), name='last_W')
...
epoch = 40
save_path = './log/weights/vgg_19_fine_tune.ckpt'
log_path = './log/summaries/vgg_19/'
extract = False
step = 0
loadw = False
train = True
...
with tf.Session() as sess:
    ...
    if train:
        ...
        train, test = images[:-70], images[-70:]
