...
epoch = 40
save_path = './log/weights/cnn_f_PPMD_fine_tune.ckpt'
log_path = './log/summaries/vgg_cnn_f/PPMD/'
extract = True
step = 0
loadw = True
train = False
...
with tf.Session() as sess:
    ...
    if loadw:
        ...
        im_path = './data/test/PPMD/'
        if extract:
            images = [
                [mp.imread('./data/test/PPMD/' + name), get_class_number(name), name]
                for name in os.listdir('./data/test/PPMD/')
            ]
            features = []
            ...
            features.to_csv('CNNF_features_PPMD_test.csv', index=False)
