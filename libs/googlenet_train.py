...
epoch = 40
save_path = './log/weights/googlenet-fine_tune.ckpt'
log_path = './log/summaries/'
extract = True
step = 396
loadw = True
train = False
        ...
        if extract:
            ...
            for c in images.keys():
                for k in images[c].keys():
                    afs = sess.run(loss3_classifier_SOD, feed_dict={image: [images[c][k]]})
                    for af in afs:
                        features.append(list(af) + [c] + [c+'_'+k])
            features = pd.DataFrame(features, columns=[str(i) for i in range(100)] + ['label'] + ['name'])
            features.to_csv('GoogleNetFeatures1.csv', index=False)
