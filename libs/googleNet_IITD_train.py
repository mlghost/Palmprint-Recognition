...
epoch = 40
save_path = './log/weights/googlenet-IITD-fine_tune.ckpt'
...
with tf.Session() as sess:
    ...
    if loadw:
        ...
        im_path = './data/test/IITD/'
        if extract:
            images = [
                [mp.imread('./data/test/IITD/' + name), get_class_number(name),name]
                for name in os.listdir('./data/test/IITD/')
            ]
            features = []
            ...
            features.to_csv('GoogleNet_features_IITD_test.csv', index=False)
